...
function closeOtherProblemTabs(currentFilePath, workspaceFolder) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a;
        const tabGroups = (_a = vscode.window.tabGroups) === null || _a === void 0 ? void 0 : _a.all;
        if (!Array.isArray(tabGroups) || !workspaceFolder) {
            return;
        }
        ...
        for (const group of tabGroups) {
            for (const tab of group.tabs) {
                ...
                const tabPath = path.resolve(tabUri.fsPath);
                if (tabPath === normalizedCurrentPath) {
                    continue;
                }
                if (!isLeetCodeProblemFile(tabPath, normalizedWorkspaceFolder)) {
                    continue;
                }
        ...
        if (tabsToClose.length) {
            yield vscode.window.tabGroups.close(tabsToClose, true);
        }
    });
}
function getTabUri(tab) {
    ...
    if (input && input.modified) {
        return input.modified;
    }
    return undefined;
}
function isLeetCodeProblemFile(filePath, workspaceFolder) {
    const normalizedFilePath = path.resolve(filePath);
    if (!isSameOrSubPath(normalizedFilePath, workspaceFolder)) {
        return false;
    }
    return /^\d+\..+\.[^.]+$/.test(path.basename(normalizedFilePath));
}
function isSameOrSubPath(filePath, folderPath) {
